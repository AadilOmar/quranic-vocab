...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "signup") {
      ...
      if (error) { setError(error.message); } else { setSignupDone(true); }
      setSubmitting(false);
    } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-6 bg-[#FAFAF7]">
      <div className="w-full max-w-sm">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 py-3 bg-white border border-stone-200 rounded-xl text-sm font-medium text-stone-700 hover:bg-stone-50 transition-colors mb-4 shadow-sm"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input
            ...
            placeholder="Email"
            required
            className="w-full px-4 py-3 rounded-xl border border-stone-200 text-sm text-stone-700 placeholder-stone-300 focus:outline-none focus:border-amber-400 bg-white"
          />
          <input
            type="password"
            value={password}
